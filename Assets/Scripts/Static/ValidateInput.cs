...
public class ValidateInput : MonoBehaviour
{
    ...
    private TMP_InputField inputToValidate;
    ...
    void Start()
    {
        inputToValidate = GetComponent<TMP_InputField>();
    ...
    {
        print("Validate " + arg0);
        print("Validate ......" + LanguageManager.Instance.GetTextValue(blankErrorMessageKey));
        if (arg0.Length <= 0)
        {
        ...
        else if (isChangePasswordMatch && !MyAccount.instance.MatchPassword())
        {
            validationInfo.text = LanguageManager.Instance.GetTextValue(errorMessageKey); 
            isValidInput = false;
        }
